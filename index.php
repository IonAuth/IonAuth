...
$app->post('/login', function() use ($auth) {
	...
	$remember = (bool) $app->request()->post('remember');


	//verify login
	if ($auth->login($identity, $password, $remember) === TRUE) {
...
//change password
$app->get('/change_password', function() use ($auth) {
...
$app->post('/change_password', function() use ($auth) {


	$identity    = $_SESSION['identity'];
	$oldPassword = $app->request()->post('old');
	$newPassword = $app->request()->post('new');
...
$app->get('/forgot_password', function() use ($auth) {

	$email = strtolower($app->request()->post('email'));

	// change to username if you are using that as the identitiy
	...
	$forgotten = $auth->forgottenPassword($user->email);
...
$app->post('/forgot_password', function() use ($auth) {
...
$app->post('/reset_password/{code?}', function() use ($auth) {
